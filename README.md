...
6마리를 데려갔다면 한 마리는 메가진화, 다른 한 마리는 거다이맥스, 또 한 마리는 Z기술 —
여섯 마리가 모두 거다이맥스할 수는 없다. 방 설정에서 각각 끌 수 있다.
...
| | 자격 | 효과 |
|---|---|---|
| **메가진화** | 메가 폼이 있는 종만 | 타입·종족값이 메가 폼으로 교체 (HP는 원작대로 유지). 배틀 끝까지 지속 |
| **거다이맥스** | 거다이맥스 폼이 있는 종만 | 최대 HP ×1.5, **3턴**간 모든 기술이 맥스 기술로. 끝나면 HP 상한 원복 |
| **Z기술** | 공격기가 있으면 누구나 (도구 기반) | 그 턴 공격 한 번이 Z기술로. 필중 |

리자몽처럼 메가 폼이 둘인 경우(X/Y) 어느 쪽인지 고를 수 있다.

**님의 현재 로스터는 해당 대상이 없다**: 쥬레곤(87)과 꿀꺽몬(317)은 메가·거다이맥스 폼이
둘 다 없다. Z기술만 쓸 수 있다. 리자몽·팬텀·잠만보·후딘 등을 모으면 쓸 수 있게 된다.
...
$B --formtest     # 메가/거다이맥스/Z — 자격 제한, 배틀당 1회, 지속턴, 위력 변환표
$B --pickertest   # 포켓몬 선택 — 부분 선택 존중, 선봉/교체 인덱스, 범위 초과 처리
$B --nettest      # Bonjour 광고/발견, TXT 레코드, TCP 프레이밍, 직렬화 왕복, 정원 초과 거절
